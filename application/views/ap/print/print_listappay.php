...
#die('tes');
		...
		//(trx_amt*10)/100 as PPN, (trx_amt*10)/100 as PPH23
			...
			$data = $this->db->query("sp_PaymentAP '".$vendor."','".$project_detail."','".$checkbox."','".inggris_date($startdate)."','".inggris_date($enddate)."'")
							 ->result();
	...
	$a = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa4";
			...
			$pdf->Cell(45,8,$row->nm_supplier,1,0,'L',0);
			$pdf->Cell(20,8,$row->doc_no,1,0,'L',0);
			$pdf->Cell(20,8,$row->doc_date,1,0,'L',0);
			$pdf->Cell(65,4,substr($row->descs,0,58),"L"."R",0,'L',0);
			$pdf->Cell(25,8,number_format($row->amount),1,0,'R',0);
			$pdf->Cell(35,6,substr($row->bank_nm,0,26),"L"."R",0,'L',0);
			$pdf->Cell(25,8,$row->paidby,1,0,'C',0);
