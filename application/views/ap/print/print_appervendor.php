<?php
		...
		//die('a');
			...
			if(!empty($cb)){
			foreach($cb as $row){
			$pdf->Cell(8,5,$nos,1,0,'C',0);
			$pdf->Cell(30,5,$row->doc_no,1,0,'C',0);
			$pdf->Cell(30,5,substr($rowk->nm_supplier,0,21),1,0,'L',0);
			$pdf->Cell(30,5,indo_date($row->due_date),1,0,'C',0);
			$pdf->Cell(150,5,$row->descs,1,0,'L',0);
			...
			$pdf->Cell(20,5,'',10,0,'C',0);
			$pdf->Cell(20,5,'',10,0,'C',0);
			$pdf->Cell(105,5,'GRAND TOTAL',1,0,'L',1);
			$pdf->Cell(20,5,number_format($tsa),1,0,'R',1);
