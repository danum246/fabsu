...
//		$voucher = $this->input->
	...
	function getaccname($id){
		...
		function view($id){
			$session_id = $this->UserLogin->isLogin();
			$pt	= $session_id['id_pt'];
			$cek = $this->db->query("select trx_type from db_apinvoice where apinvoice_id = ".$id."")->row();
			if (($pt == 11) and ($cek->trx_type == 'MAN')) {
				$data['detail'] = $this->db->query("select * from db_apinvoice where apinvoice_id = ".$id." ")->row();
				$data['vendor'] = $this->db->select('kd_supp_gb,nm_supplier')
					->join('db_subproject','kd_project = subproject_id')
					->where('pt_id',$pt)
					->order_by('nm_supplier','ASC')
